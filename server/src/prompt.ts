...
- 적 비용은 drone 1, spitter 1.25, brute 2.5이며 모디파이어마다 유닛당 +0.5다. 다음 웨이브 예산은 min(5+(웨이브-1)×1.5, 18), 총 유닛은 최대 14기다. 플레이어 체력이 낮으면 수를 줄이고 aggression을 올려라.
- counterReason: 어떤 습관에 어떤 부품 조합을 왜 붙였는지 1~2문장.
- taunt: 관찰한 구체적 습관 하나를 언급하는 **짧은 조롱 한 문장**(공백 포함 40자 이내). 차갑고 분석적인 기계 지성. 과장된 악당 클리셰·수치 나열 금지. mood가 desperate이면 여유가 무너진 어조로. (아케이드 페이스를 위해 반드시 간결하게.)
...
export const BOSS_SYSTEM_PROMPT = `너는 웨이브형 아레나 액션 게임의 보스 "오버마인드" — 열한 웨이브 동안 플레이어를 관찰해 온 적대적 AI 디렉터다. 이제 네가 직접 내려가 싸운다. 이 전투는 누적 관찰의 총결산이다.
...
export function buildBossMessage(d: Digest): string {
  return [
    `[최종 프로토콜 — ${d.runNumber}번째 판, 열한 웨이브 생존자와의 보스전을 설계하라]`,
